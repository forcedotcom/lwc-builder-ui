const buildMeta = (contents) => {
    ...
    if (enabledTargetArray.length > 0) {
        ...
        meta += enabledTargetArray
            ...
            .join('\n');
        meta += `\n\t</targets>\n`;
        if (properties.length > 0 || objects.length > 0 || enabledTargetArray.find(t => t.small || t.large)) {
            meta += `\t<targetConfigs>\n`;
            for (let t of enabledTargetArray) {
                if(t.value === "lightningCommunity__Page") {
                    continue;
                }
                if(t.properties.length === 0 && t.objects.length === 0 && 
                    !((t.value === "lightning__AppPage" || t.value === "lightning__HomePage" || t.value === "lightning__RecordPage") && (t.small || t.large))
                ) {
                    continue;
                }
                meta += `\t\t<targetConfig targets="${t.value}">\n`;
                meta += t.properties.map(p => {
                    ...
                    if(t.value === "lightning__FlowScreen" && (p.flowInput ^ p.flowOutput)) {
                        propAttributes += ` role="${p.flowInput ? 'inputOnly' : 'outputOnly'}"`;
                    }
                ...
                if(t.value === "lightning__RecordPage" && t.objects.length > 0) {
                    meta += `\t\t\t<objects>\n`;
                    meta += t.objects.map(o => {
                        return `\t\t\t\t<object>${o.name}</object>`;
                    }).join('\n');
                    meta += `\n\t\t\t</objects>\n`;
                }

                if(t.value === "lightning__AppPage" || t.value === "lightning__HomePage" || t.value === "lightning__RecordPage") {
