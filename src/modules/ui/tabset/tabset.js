...
import { generateUniqueId } from 'ui/inputUtils';
...
export default class Tabset extends LightningElement {
  ...
  _selectTabHeaderByTabValue(value) {
    ...
    const tabBar = this.template.querySelector('ui-tab-bar');
  ...
  _updateTabBarHeaders(headers) {
    this.template.querySelector('ui-tab-bar').tabHeaders = headers.slice();
  }
  ...
  @api
  focus() {
    this.template.querySelector('ui-tab-bar').focus();
  }
}
